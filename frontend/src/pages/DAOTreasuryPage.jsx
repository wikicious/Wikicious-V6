...
export default function DAOTreasuryPage() {
  ...
  const { data:stats={} } = useQuery({queryKey:['DAOTreasuryPage'],queryFn:()=>api('/api/dao-treasury/stats'),placeholderData:{}});
  return (
    <AppLayout>
      <div style={{maxWidth:1100,margin:'0 auto',padding:'28px 20px'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24}}>
          ...
          <div><h1 style={{margin:0,fontSize:26,fontWeight:900,color:S.t1}}>DAO Treasury Management</h1>
          <p style={{margin:0,color:S.t3,fontSize:13}}>Manage idle DAO treasuries on Arbitrum. 0.5% management + 10% performance fee. $120K+/year per $10M client.</p></div>
        </div>
        <div style={{background:'#FFB8000D',border:'1px solid #FFB80030',borderRadius:10,padding:'12px 16px',marginBottom:22}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:24}}>
          {Object.entries(stats).slice(0,6).map(([k,v])=>(
            <div key={k} style={{background:S.bg,border:`1px solid ${S.b}`,borderRadius:12,padding:'14px 16px'}}>
              ...
              <div style={{color:'#FFB800',fontSize:18,fontWeight:900,fontFamily:'monospace'}}>{typeof v==='number'?v.toLocaleString():fmtM(v?.toString()||'0')}</div>
            </div>
          ))}
        </div>
        <div style={{background:S.bg,border:`1px solid ${S.b}`,borderRadius:14,padding:22}}>
          ...
          <div style={{display:'flex',gap:10}}>
            <button style={{padding:'12px 28px',borderRadius:10,border:'none',cursor:address?'pointer':'not-allowed',fontWeight:900,fontSize:13,background:address?'#FFB800':'#1C2138',color:address?'#000':'#4A5270'}}>
