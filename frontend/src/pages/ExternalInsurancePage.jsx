...
export default function ExternalInsurancePage() {
  const { address } = useAccount();
  const [amount, setAmount] = useState('');
  const { data:stats={} } = useQuery({queryKey:['ExternalInsurancePage'],queryFn:()=>api('/api/ext-insurance/stats'),placeholderData:{}});
  return (
    <AppLayout>
      <div style={{maxWidth:1100,margin:'0 auto',padding:'28px 20px'}}>
        <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24}}>
          ...
          <div><h1 style={{margin:0,fontSize:26,fontWeight:900,color:S.t1}}>External Protocol Insurance</h1>
          <p style={{margin:0,color:S.t3,fontSize:13}}>Sell liquidation backstop coverage to other DeFi protocols on Arbitrum. Charge 0.25% of covered TVL per year.</p></div>
        </div>
        <div style={{background:'#5B7FFF0D',border:'1px solid #5B7FFF30',borderRadius:10,padding:'12px 16px',marginBottom:22}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,marginBottom:24}}>
          {Object.entries(stats).slice(0,6).map(([k,v])=>(
            <div key={k} style={{background:S.bg,border:`1px solid ${S.b}`,borderRadius:12,padding:'14px 16px'}}>
              ...
              <div style={{color:'#5B7FFF',fontSize:18,fontWeight:900,fontFamily:'monospace'}}>{typeof v==='number'?v.toLocaleString():fmtM(v?.toString()||'0')}</div>
            </div>
          ))}
        </div>
        <div style={{background:S.bg,border:`1px solid ${S.b}`,borderRadius:14,padding:22}}>
          ...
          <div style={{display:'flex',gap:10}}>
            <button style={{padding:'12px 28px',borderRadius:10,border:'none',cursor:address?'pointer':'not-allowed',fontWeight:900,fontSize:13,background:address?'#5B7FFF':'#1C2138',color:address?'#000':'#4A5270'}}>
