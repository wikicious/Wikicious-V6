...
///   2. Trader opens position → interface IFlashLoanReceiver {
...
contract WikiPropFunded is Ownable2Step, ReentrancyGuard, Pausable, IFlashLoanReceiver {
    ...
    function openPosition(
        ...
        require(
            ...
            string(abi.encodePacked(
                ...
                "x (Tier ",
                _uint2str(acc.tier),
                " funded cap). Eval was higher — funded uses real pool capital."
            ))
        );
    ...
    function getWithdrawalStatus(uint256 accountId) external view returns (
        ...
        statusMsg         = canWithdrawNow
            ? "Ready to withdraw"
            : "Cooldown active — wait for next withdrawal window";
    }
